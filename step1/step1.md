...
[trinket.strivemath.org/console](https://trinket.strivemath.org/console)
...
The last one gives an error because we are accidentally mixing different types of objects in the addition sum: ```"20"``` is a text string and ```24``` is a number. You can add together objects of the *same* type, or different types of numbers,  but you can't mix a number type and a text string in an addition sum.
